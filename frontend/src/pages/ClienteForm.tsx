...
export default function ClienteForm() {
  ...
  useEffect(() => {
    setActions({
      ...
      canDelete: !isNew,
      canReset: formDirty && !isNew,
      isSaving: saveMutation.isPending,
    })
    return () => clearActions()
  }, [form.ragione_sociale, formDirty, isNew, saveMutation.isPending, cliente, resetForm])
        ...
        <div className="max-w-2xl">
          ...
          {sezione === 2 && (
            <div className="space-y-4">
              ...
              <Field label="PEC Fatturazione Elettronica"><Input type="email" value={form.fatturazione_elettronica?.pec_fe ?? ''} onChange={e => set('fatturazione_elettronica.pec_fe', e.target.value)} placeholder="fe@azienda.it" /></Field>
          ...
          {sezione === 3 && (
            <div className="space-y-4">
              ...
              {(form.contatti ?? []).length === 0 && <p className="text-sm text-slate-400 py-4 text-center">Nessun contatto aggiunto</p>}
              {(form.contatti ?? []).map((c, i) => (
                <div key={i} className="border border-slate-200 rounded-xl p-4 space-y-3 bg-slate-50/50">
                  <div className="flex items-center justify-between">
                    ...
                    <button onClick={() => removeContatto(i)} className="p-1.5 text-slate-400 hover:text-red-500 rounded transition"><Trash2 size={14} /></button>
                  </div>
                  <div className="grid grid-cols-3 gap-3">
                    <Field label="Nome"><Input value={c.nome} onChange={e => setContatto(i, 'nome', e.target.value)} placeholder="Mario Rossi" /></Field>
                    <Field label="Telefono"><Input value={c.telefono ?? ''} onChange={e => setContatto(i, 'telefono', e.target.value)} placeholder="+39 339..." /></Field>
                    <Field label="Email"><Input value={c.email ?? ''} onChange={e => setContatto(i, 'email', e.target.value)} placeholder="m.rossi@..." /></Field>
                  </div>
                </div>
              ))}
            </div>
          ...
          {sezione === 4 && (
            <div className="space-y-4">
              ...
              <Field label="Istituto di credito"><Input value={form.pagamento?.istituto ?? ''} onChange={e => set('pagamento.istituto', e.target.value)} placeholder="Banca Intesa" /></Field>
              <div className="grid grid-cols-3 gap-4">
                <Field label="ABI"><Input value={form.pagamento?.abi ?? ''} onChange={e => set('pagamento.abi', e.target.value)} placeholder="01234" /></Field>
                <Field label="CAB"><Input value={form.pagamento?.cab ?? ''} onChange={e => set('pagamento.cab', e.target.value)} placeholder="56789" /></Field>
                <Field label="C/C"><Input value={form.pagamento?.cc ?? ''} onChange={e => set('pagamento.cc', e.target.value)} placeholder="000012345678" /></Field>
              </div>
              <Field label="IBAN"><Input value={form.pagamento?.iban ?? ''} onChange={e => set('pagamento.iban', e.target.value)} placeholder="IT60 X054 2811 1010 0000 0123 456" /></Field>
            </div>
          )}
