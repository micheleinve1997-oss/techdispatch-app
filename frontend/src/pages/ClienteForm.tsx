        ...
        <div>
          ...
          {editMode && !isNew && (
            <p className="text-xs text-blue-600 mt-0.5 flex items-center gap-1">
              <Pencil size={11} /> ModalitÃ  modifica attiva
            </p>
          )}
        </div>
        {!isNew && cliente && (
          ...
          {SEZIONI.map((s, i) => (
            <button key={s} onClick={() => setSezione(i)}
              className={`px-4 py-3 text-sm font-medium border-b-2 rounded-t-lg transition-colors whitespace-nowrap ${
                sezione === i ? 'border-blue-600 text-blue-600 bg-blue-50/60' : 'border-transparent text-slate-500 hover:text-slate-700 hover:bg-slate-50'
              }`}>
              {s}
            </button>
          ))}
          ...
          {sezione === 0 && (
            <div className="space-y-5">
              {editMode ? (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 2xl:grid-cols-3 gap-x-12 gap-y-8">
